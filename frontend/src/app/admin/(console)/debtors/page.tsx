...
import { TD_NUM, TD_T1, TD_T2, UNIT } from '@/components/admin/styles';
          ...
          <TableWrap>
            <thead>
              <tr>
                <th>Boat</th>
                <th>Status</th>
                <th className={TD_NUM}>Balance</th>
              </tr>
            </thead>
            {isLoading ? (
              ...
              <tbody>
                {(data?.negativeBalances ?? []).map((d) => (
                  <tr key={d.id}>
                    <td className={TD_T1}>{d.houseboat.name}</td>
                    <td>
                    ...
                    <td className={TD_NUM} style={{ color: 'var(--danger)' }}>
                      <span className={UNIT}>৳</span> {formatBDT(d.platformBalance)}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {isLoading ? (
              ...
              <tbody>
                {(data?.overdueInvoices ?? []).map((inv) => (
                  <tr key={inv.id}>
                    <td className={TD_T1}>{inv.houseboat.name}</td>
                    <td>{inv.period}</td>
                    <td className={TD_NUM}>
                      <span className={UNIT}>৳</span> {formatBDT(inv.amountDue)}
                    </td>
                    <td className={TD_T2}>
